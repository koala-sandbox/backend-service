...
import (
	...
	api "github.com/koala-sandbox/backend-service/api"
)
...
func HelloKoala(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		fmt.Fprintln(w, "Error reading VERSION file (backend-service): "+err.Error())
		return
	}
	fmt.Fprintln(w, "Hello world! service:backend-service version: "+string(VERSION))
}
